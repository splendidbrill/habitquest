        ...
        {alerts.length > 0 && (
          <View style={s.alertsBox}>
            {alerts.slice(0, 2).map((alert, i) => {
              const isStreakRisk =
                alert.type === 'warning' && alert.message.includes('streak');
              ...
              return (
                ...
                >
                  <View
                    style={[
                      s.alertRow,
                      alert.type === 'warning' ? s.alertWarning : s.alertInfo,
                    ]}
                  >
                    {alert.type === 'warning' ? (
            ...
            <View style={s.card}>
              ...
              <View style={s.focusNote}>
                <Text style={s.focusNoteText}>
                  🎯 This week's focus:{' '}
                  <Text style={s.focusNoteStrong}>
                    {PILLAR_CONFIG[focusPillar].label}
                  </Text>
                </Text>
              </View>
            </View>

            {/* Parent tools — consolidated directly below the pillars (§2.5),
                instead of being buried behind "Full Dashboard". */}
            <View style={s.card}>
              <Text style={s.cardTitle}>Tools & Support</Text>
              <View style={s.toolsGrid}>
                {[
                  ...
                  {
                    emoji: '🌟',
                    label: 'Log Real-World Win',
                    onPress: () => navigation.navigate('RealWorldLog'),
                  },
                  {
                    emoji: '📊',
                  ...
                  },
                  {
                    ...
                    onPress: () =>
                      navigation.navigate('FamilyChallengesManager'),
                  },
                  {
                    emoji: '🧩',
                  ...
                  {
                    emoji: '🍽️',
                    label: 'Handling New Foods',
                    onPress: () => navigation.navigate('HandlingResistance'),
                  },
                  {
                    emoji: '💬',
                    label: 'Supportive Replies',
                    onPress: () => navigation.navigate('SupportiveResponses'),
                  },
                  {
                    emoji: '🌱',
                    label: 'Tricky Moments',
                    onPress: () => navigation.navigate('DifficultBehaviors'),
                  },
                ].map(tool => (
                  <TouchableOpacity
                    key={tool.label}
              ...
              >
                ...
                >
                  ...
                  <View style={s.checkinNudgeText}>
                    <Text style={s.checkinNudgeTitle}>
                      Weekly check-in needed
                    </Text>
                    <Text style={s.checkinNudgeSub}>
                      Update {child.name}'s scores — takes 2 minutes
                    </Text>
                  </View>
            ...
            {journey && (
              <View style={s.journeyCard}>
                <LinearGradient
                  ...
                  style={s.journeyHeader}
                >
                  <View>
                    <Text style={s.journeyPhaseLabel}>
                      Phase {journey.phase}
                    </Text>
                    <Text style={s.journeyPhaseName}>{journey.phaseLabel}</Text>
                  </View>
                  <View style={s.journeyWeekBadge}>
                    <Text style={s.journeyWeekText}>
                      Week {journey.weekNumber}
                    </Text>
                  </View>
                </LinearGradient>
                    ...
                    <View key={i} style={s.activityRow}>
                      ...
                      <View style={s.activityText}>
                        ...
                        <Text style={s.activityMeta}>
                          {pillarCfg.label} · +{m.xp_earned} XP ·{' '}
...
const PHASE_GOALS: Record<number, string[]> = {
  1: [
    'Establish breakfast routine',
    'Build daily water habit',
    'Start activity streak',
  ],
  2: [
    'Introduce new foods',
    'Build family movement habit',
    'Encourage independent choices',
  ],
  3: [
    'Maintain consistency across all pillars',
    'Build resilience and self-confidence',
    'Long-term habit mastery',
  ],
};
