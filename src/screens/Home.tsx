...
import { getOnboardingData } from '../utils/personalization';
...
export function HomeScreen() {
  ...
  const [weeklyFocus, setWeeklyFocus] = useState({
    ...
    description:
      'Adding colour to meals – no pressure, just exploring new foods together',
  });
      ...
      if (data?.['2']) {
        ...
        if (goals.includes('Create more balanced family meals')) {
          setWeeklyFocus({
            title: "This week's focus",
            description:
              'Building balanced plates – adding protein, veg, and grains to meals you already love',
          });
        } else if (goals.includes('Support my child to be more active')) {
          setWeeklyFocus({
            title: "This week's focus",
            description:
              'Moving more as a family – finding fun ways to be active together',
          });
        } else if (goals.includes('Encourage my child to eat more variety')) {
          setWeeklyFocus({
            title: "This week's focus",
            description:
              'Trying new foods – exploring one new ingredient this week, no pressure',
          });
        }
      }

      // "Today's plan" mirrors the weekly plan (single source of truth) so the
      // Home tab and the Plan tab always agree for the current day.
      const todays = await getTodaysPlan();
      const tasks: any[] = [
        {
          id: 1,
          icon: Apple,
    ...
    >
      ...
      >
        <Text style={styles.encourageText}>
          <Text style={{ fontWeight: '600' }}>Brilliant!</Text> You've ticked
          off activities this week. Remember, every small step counts and you're
          doing great.
        </Text>
      </Card>
      ...
      <Card style={styles.tipsCard}>
        ...
        <Text style={styles.tipCardDesc}>
          Let your child help in the kitchen - even small tasks like washing
          vegetables or stirring makes them more likely to try new foods.
        </Text>
        <Button
          variant="outline"
          size="sm"
