...
import { KIDS8_GRADIENTS } from './kids8Theme';
...
export function Kids8SuccessCelebration() {
  ...
  useEffect(() => {
    storage.getItem('kids8LastUnlock').then(v => {
      if (v) setLastUnlock(v);
    });
    storage.getItem('kids8CompletedMissions').then(v => {
      if (v) setCompletedMissions(parseInt(v));
    });
    storage.getItem('kids8UserName').then(v => {
      if (v) setUserName(v);
    });
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate('MysteryBox', {
                returnScreen: 'Kids8TrainingDashboard',
              })
            }
          >
            <LinearGradient
