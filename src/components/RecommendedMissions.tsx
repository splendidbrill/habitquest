...
import { getRecommendedMissions, type Recommendation } from '../services/recommendationService';
...
export function RecommendedMissions({ isDark = false }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const recs = await getRecommendedMissions(activeChild.id, ageGroup, startDate);
  ...
  return (
    <View style={[s.container, isDark && s.containerDark]}>
      ...
      <View style={s.header}>
        ...
        <Text style={[s.headerTitle, { color: textColor }]}>Recommended for you</Text>
      ...
      >
        <LinearGradient
          colors={PILLAR_COLORS[recommendations[0].pillar] ?? ['#f97316', '#ea580c']}
          style={s.primaryCard}
        ...
        >
          <View style={s.primaryCardContent}>
            ...
            <View style={s.primaryText}>
              ...
              <Text style={s.primarySubtitle}>{recommendations[0].subtitle}</Text>
        ...
        <View style={s.secondaryRow}>
          {recommendations.slice(1).map(rec => (
            ...
            >
              ...
              <Text style={[s.secondaryTitle, { color: textColor }]} numberOfLines={2}>
                {rec.title}
              </Text>
              <View style={[s.pillarDot, { backgroundColor: PILLAR_COLORS[rec.pillar]?.[0] }]} />
            </TouchableOpacity>
          ))}
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  primarySubtitle: { fontSize: 13, color: 'rgba(255,255,255,0.85)', marginBottom: 8 },
  ...
  startRow: {
    ...
    gap: 4, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 50,
    paddingVertical: 10,
  },
  ...
  secondaryCardDark: { backgroundColor: 'rgba(255,255,255,0.08)', borderColor: 'rgba(255,255,255,0.1)' },
